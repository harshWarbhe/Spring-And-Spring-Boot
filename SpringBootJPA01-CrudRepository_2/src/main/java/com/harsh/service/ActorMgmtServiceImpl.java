...
import com.harsh.error.ActorNotFoundException;
...
public class ActorMgmtServiceImpl implements IActorMgmtService {
	...
	public String registerActor(Actor actor) {
		System.out.println("actorRepo obj class name::" + actorRepo.getClass() + " .....\r\n  "
				...
				+ Arrays.toString(actorRepo.getClass().getDeclaredMethods()));
	...
	/*
	 * @Override public String registerActors(List<Actor> list) { //use repo
	 * Iterable<Actor> actorsList=actorRepo.saveAll(list); List<Integer> ids = new
	 * ArrayList(); actorsList.forEach(actor->{ ids.add(actor.getAid()); });
	 * 
	...
	public String registerActors(List<Actor> list) {
		...
		List<Integer> ids = StreamSupport.stream(actorsList.spliterator(), false).map(Actor::getAid)
	...
	public Iterable<Actor> showAllActors() {
		...
		List<Actor> list1 = StreamSupport.stream(list.spliterator(), false)
				.sorted((a1, a2) -> a1.getAname().compareTo(a2.getAname())).collect(Collectors.toList());
		return list1;
	}
